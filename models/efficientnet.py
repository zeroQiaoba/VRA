...
class EfficientNet(nn.Module):
    def __init__(
        ...
        last_channel: Optional[int] = None,p=None, info=None, clip_threshold=1e10,
        data_type='No',lamb = 0,featmin=0,featmax=0,
        **kwargs: Any,
    ) -> None:
        """
    ...
    def _forward_impl(self, x: Tensor,feat=False) -> Tensor:
        x = self.features(x)
        ...
        if feat == True:
            return x
        if self.case == 'clip2':
            x = torch.where(x<self.featmin,torch.zeros(x.shape).cuda()+0,x)
            x = torch.where(x>self.featmax,torch.zeros(x.shape).cuda()+self.featmax,x)

        x = self.classifier(x)

        return x

    def forward(self, x: Tensor,feat=False) -> Tensor:
        return self._forward_impl(x,feat=feat)
