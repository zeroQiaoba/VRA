...
class RegNet(nn.Module):
    def __init__(
        ...
        activation: Optional[Callable[..., nn.Module]] = None,p=None, info=None, clip_threshold=1e10,
    ...
    def forward(self, x: Tensor,feat=True) -> Tensor:
        x = self.stem(x)
        ...
        if self.case == 'clip2':
            x = x+self.lamb
            x = torch.where(x<self.featmin+self.lamb,torch.zeros(x.shape).cuda()+0,x)
            x = torch.where(x>self.featmax+self.lamb,torch.zeros(x.shape).cuda()+self.featmax,x)
        x = self.fc(x)
        return x
