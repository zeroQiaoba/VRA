...
class DenseNet3(nn.Module):
    def __init__(self, depth, num_classes, growth_rate=12,
                 ...
                 out_classes = 100, p=None, info=None,data_type='clip2',lamb = 0,featmin=0,featmax=0):
    ...
    def forward(self, x,feat = False):
        ...
        if self.case == 'clip2':
            out = out+self.lamb
            out = torch.where(out<(self.featmin+self.lamb),torch.zeros(out.shape).cuda()+0,out)
            out = torch.where(out>(self.featmax+self.lamb),torch.zeros(out.shape).cuda()+self.featmax,out)
        out = self.fc(out)
        if feat == 'VRA++':
            return out0,out
        return out
